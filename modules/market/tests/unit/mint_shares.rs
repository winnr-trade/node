...
fn test_mint_shares_multiple_times() {
    ...
    assert_eq!(position.no_shares, 350);
...
fn test_mint_shares_different_users() {
    ...
    assert_eq!(user_pos.yes_shares, 100);
    assert_eq!(user_pos.no_shares, 100);

    let admin_pos = get_position(&runner, market_id, &admin).expect("admin position should exist");
    assert_eq!(admin_pos.yes_shares, 200);
    assert_eq!(admin_pos.no_shares, 200);
